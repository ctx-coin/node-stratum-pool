...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData){
	...
	/* CTX */
if (rpcData.acceptancepointsvalue && rpcData.acceptancepointsScript) {
    reward -= rpcData.acceptancepointsvalue;
    console.log(reward);
    rewardToPool -= rpcData.acceptancepointsvalue;
}
		
if (rpcData.poscoachesvalue && rpcData.poscoachesScript) {
    reward -= rpcData.poscoachesvalue;
    console.log(reward);
    rewardToPool -= rpcData.poscoachesvalue;
}

if (rpcData.devmarketingvalue && rpcData.devmarketingScript) {
    reward -= rpcData.devmarketingvalue;
    console.log(reward);
    rewardToPool -= rpcData.devmarketingvalue;
}
...
if (rpcData.payee) {
	...
	/* CTX */
	if (rpcData.acceptancepointsvalue && rpcData.acceptancepointsScript) {
	console.log(rpcData.acceptancepointsvalue)
	console.log(rpcData.acceptancepointsScript)
        var acceptancepointsScript = util.miningKeyToScript(rpcData.acceptancepointsScript);
        txOutputBuffers.unshift(Buffer.concat([
            util.packInt64LE(rpcData.acceptancepointsvalue),
            util.varIntBuffer(acceptancepointsScript.length),
            acceptancepointsScript
        ]));
    }
	
	if (rpcData.poscoachesvalue && rpcData.poscoachesScript) {
	console.log(rpcData.poscoachesvalue)
	console.log(rpcData.poscoachesScript)
        var poscoachesScript = util.miningKeyToScript(rpcData.poscoachesScript);
        txOutputBuffers.unshift(Buffer.concat([
            util.packInt64LE(rpcData.poscoachesvalue),
            util.varIntBuffer(poscoachesScript.length),
            poscoachesScript
        ]));
    }
	
	if (rpcData.devmarketingvalue && rpcData.devmarketingScript) {
	console.log(rpcData.devmarketingvalue)
        var devmarketingScript = util.miningKeyToScript(rpcData.devmarketingScript);
        txOutputBuffers.unshift(Buffer.concat([
            util.packInt64LE(rpcData.devmarketingvalue),
            util.varIntBuffer(devmarketingScript.length),
            devmarketingScript
        ]));
    }
    return Buffer.concat([
    ...
    console.log(txOutputBuffers.length)
};
